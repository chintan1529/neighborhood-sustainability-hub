...
serve(async (req) => {
    ...
    try {
        ...
        const idempotencyKey = body.idempotency_key || `${user.id}-${body.report_id}-${Date.now()}`;
        ...
        const { data: report } = await supabaseUser
            .from("waste_reports")
            .select("id, latitude, longitude, address_text, landmark, notes, confirmed_class, predicted_class, photo_url")
