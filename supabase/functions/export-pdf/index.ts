      ...
      `)
        ...
        (collectorClaims || []).forEach((c: any) => {
            ...
            if (!collectorStats[id]) {
                collectorStats[id] = { name: c.profiles?.full_name || "Unknown", completed: 0 };
            }
        ...
        completedReports.forEach((r) => {
            const hours = (new Date(r.completed_at).getTime() - new Date(r.created_at).getTime()) / 3600000;
            totalHours += hours;
        });
        ...
        const stats: MonthlyStats = {
            ...
            completion_rate: allReports.length > 0 ? (completedReports.length / allReports.length) * 100 : 0,
            avg_completion_hours: completedReports.length > 0 ? totalHours / completedReports.length : 0,
            sla_compliance: completedReports.length > 0 ? (slaCompliant.length / completedReports.length) * 100 : 0,
        ...
        const monthYear = startDate.toLocaleDateString("en-IN", { month: "long", year: "numeric" });
