  ...
  try {
    ...
    if (!profile.neighborhood_id) {
      return new Response(
        JSON.stringify({
          success: false,
          error: "User must be assigned to a neighborhood",
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    const base64Data = body.photo_base64.replace(
      /^data:image\/\w+;base64,/,
      "",
    );
    ...
    const {
      data: { publicUrl },
    } = supabaseAdmin.storage.from("report-photos").getPublicUrl(storagePath);
