...
serve(async (req) => {
    ...
    try {
        ...
        if (!body.report_id || !body.status) {
            return new Response(
                JSON.stringify({ success: false, error: "report_id and status are required" }),
        ...
        const isAssignedCollector = profile.role === "collector" && report.assigned_to === user.id;
        const isReportOwner = report.user_id === user.id;

        if (!isAdmin && !isAssignedCollector && !isReportOwner) {
            return new Response(
                JSON.stringify({ success: false, error: "You don't have permission to update this report" }),
        ...
        if (body.status === "completed") {
            if (!isAssignedCollector && !isAdmin) {
                return new Response(
                    JSON.stringify({ success: false, error: "Only assigned collector can complete" }),
                    { status: 403, headers: { ...corsHeaders, "Content-Type": "application/json" } }
                );
            }
            ...
            if (completeError) {
                ...
                return new Response(
                    JSON.stringify({ success: false, error: "Failed to complete report" }),
                    { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }
                );
            }

            const dbResult = result as { success: boolean; points_awarded?: number; error?: string; message?: string };
