...
describe("Waste type factor", () => {
  it("should rank organic waste higher than paper", () => {
    ...
    const organicResult = calculatePriority(
      {
        id: organic.id,
        wasteType: organic.confirmed_class,
        createdAt: organic.created_at,
      },
      [],
    );
    const paperResult = calculatePriority(
      {
        id: paper.id,
        wasteType: paper.confirmed_class,
        createdAt: paper.created_at,
      },
      [],
    );

    expect(organicResult.breakdown.wasteTypeScore).toBeGreaterThan(
      paperResult.breakdown.wasteTypeScore,
    );
  });
  ...
  it("should use predicted_class if preferred", () => {
    ...
    const result = calculatePriority(
      {
        id: report.id,
        wasteType: report.predicted_class,
        createdAt: report.created_at,
      },
      [],
    );
...
describe("Age (SLA) factor", () => {
  it("should score new reports lower than old reports", () => {
    ...
    const newResult = calculatePriority(
      {
        id: newReport.id,
        wasteType: newReport.confirmed_class,
        createdAt: newReport.created_at,
      },
      [],
    );
    const oldResult = calculatePriority(
      {
        id: oldReport.id,
        wasteType: oldReport.confirmed_class,
        createdAt: oldReport.created_at,
      },
      [],
    );

    expect(oldResult.breakdown.ageScore).toBeGreaterThan(
      newResult.breakdown.ageScore,
    );
  });
  ...
  it("should max out at SLA breach", () => {
    ...
    const result = calculatePriority(
      {
        id: breachedReport.id,
        wasteType: breachedReport.confirmed_class,
        createdAt: breachedReport.created_at,
      },
      [],
    );
...
describe("Quantity factor", () => {
  it("should score large quantities higher than small", () => {
    ...
    const largeResult = calculatePriority(
      {
        id: large.id,
        wasteType: large.confirmed_class,
        createdAt: large.created_at,
        quantity: large.quantity_estimate,
      },
      [],
    );
    const smallResult = calculatePriority(
      {
        id: small.id,
        wasteType: small.confirmed_class,
        createdAt: small.created_at,
        quantity: small.quantity_estimate,
      },
      [],
    );

    expect(largeResult.breakdown.quantityScore).toBeGreaterThan(
      smallResult.breakdown.quantityScore,
    );
  });
});
...
describe("Density factor", () => {
  it("should increase score when nearby reports exist", () => {
    ...
    const nearbyReports = [
      makeReport({ latitude: 12.972, longitude: 77.595 }),
      makeReport({ latitude: 12.9718, longitude: 77.5948 }),
    ];

    const mappedNearby = nearbyReports.map((r) => ({
      latitude: r.latitude,
      longitude: r.longitude,
    }));

    const aloneResult = calculatePriority(
      {
        id: report.id,
        wasteType: report.confirmed_class,
        createdAt: report.created_at,
        latitude: report.latitude,
        longitude: report.longitude,
      },
    ...
    const denseResult = calculatePriority(
      {
        id: report.id,
        wasteType: report.confirmed_class,
        createdAt: report.created_at,
    ...
    expect(denseResult.breakdown.densityScore).toBeGreaterThan(
      aloneResult.breakdown.densityScore,
    );
  });
});
...
describe("AI Confidence factor", () => {
  it("should score low-confidence reports higher (needs human review)", () => {
    ...
    const lowResult = calculatePriority(
      {
        id: lowConf.id,
        wasteType: lowConf.confirmed_class,
        createdAt: lowConf.created_at,
        predictionConfidence: lowConf.prediction_confidence,
      },
      [],
    );
    const highResult = calculatePriority(
      {
        id: highConf.id,
        wasteType: highConf.confirmed_class,
        createdAt: highConf.created_at,
        predictionConfidence: highConf.prediction_confidence,
      },
      [],
    );

    expect(lowResult.breakdown.confidenceScore).toBeGreaterThan(
      highResult.breakdown.confidenceScore,
    );
  });
});
