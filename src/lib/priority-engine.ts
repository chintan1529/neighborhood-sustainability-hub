...
const WASTE_URGENCY: Record<string, number> = {
    ...
    plastic:   0.60,  // Environmentally harmful
    metal:     0.55,  // Stable but can be sharp/dangerous
    glass:     0.50,  // Safety hazard if broken
    cardboard: 0.45,  // Stable, low urgency
    paper:     0.40,  // Most stable, lowest urgency
};
...
function scoreWasteType(wasteType: string): { score: number; reason: string | null } {
...
function scoreQuantity(quantity: string | null | undefined): { score: number; reason: string | null } {
    if (!quantity) return { score: 0.5, reason: null };

    const q = quantity.toLowerCase();
    if (q.includes('large') || q.includes('many') || q.includes('bulk') ||
        /[3-9]/.test(q) || q.includes('10')) {
...
function scoreConfidence(confidence: number | null | undefined): { score: number; reason: string | null } {
...
function scoreDensity(
    ...
    for (const other of allReports) {
        ...
        const dist = haversineKm(report.latitude, report.longitude, other.latitude, other.longitude);
    ...
    return {
        score,
        reason: nearbyCount >= 2 ? `cluster of ${nearbyCount + 1} nearby reports` : null,
        nearbyCount,
    };
}
...
export function calculatePriority(
    ...
    const rawScore =
        wasteResult.score * 0.30 +
        ageResult.score * 0.30 +
        quantityResult.score * 0.15 +
        densityResult.score * 0.15 +
        confidenceResult.score * 0.10;
