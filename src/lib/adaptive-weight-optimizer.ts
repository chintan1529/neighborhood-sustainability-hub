...
export function computeConfidence(
    ...
    return FEATURE_KEYS.reduce((sum, key) => sum + features[key] * (weights[key] || 0), 0);
}

export function normalizeWeights(weights: AdaptiveWeightVector): AdaptiveWeightVector {
    const safeValues = FEATURE_KEYS.map((key) => Math.max(weights[key] || 0, 1e-6));
...
function rowToWeights(row: AdaptivePredictionWeights | null | undefined): AdaptiveWeightVector {
    if (!row) {
...
function averageFeatures(items: AdaptiveFeatureScores[]): AdaptiveWeightVector {
    ...
    const sums = FEATURE_KEYS.reduce((result, key) => {
        result[key] = 0;
        return result;
...
function getFeatureContribution(
    ...
    const verifiedMean = verifiedFeatures.length > 0 ? averageFeatures(verifiedFeatures) : zeroVector();
    const missedMean = missedFeatures.length > 0 ? averageFeatures(missedFeatures) : zeroVector();
    const actualMean =
        actualHotspotFeatures.length > 0 ? averageFeatures(actualHotspotFeatures) : verifiedMean;

    const contribution = FEATURE_KEYS.reduce((result, key) => {
        result[key] = round((0.65 * verifiedMean[key] + 0.35 * actualMean[key]) - missedMean[key]);
        return result;
    }, {} as AdaptiveWeightVector);
...
export async function optimizeAdaptiveWeights(supabase: any, args: {
    ...
    await supabase
        .from('adaptive_prediction_weight_history')
        .insert({
            iteration: nextIteration,
