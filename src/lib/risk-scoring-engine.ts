...
async function loadConfig(): Promise<RiskScoringConfig> {
    ...
    if (!data) {
        return {
            ...
            severity_weights: {
                plastic: 5, metal: 3, glass: 3, cardboard: 2,
                paper: 2, organic: 1, mixed: 2.5,
            },
        };
    }
...
export async function computeAllRiskScores(): Promise<RiskComputationResult> {
    ...
    const { data: reports } = await supabase
        .from('waste_reports')
        .select('id, latitude, longitude, created_at, completed_at, status, predicted_class, confirmed_class')
        .neq('status', 'cancelled');

    if (!reports || reports.length === 0) {
        return { zonesUpdated: 0, zonesRemoved: 0, duration: Date.now() - start, timestamp: new Date().toISOString() };
    }
    ...
    for (const [zoneId, zone] of zones) {
        ...
        const unresolvedCount = zone.reports.filter(
            (r: any) => ['pending', 'assigned', 'in_progress'].includes(r.status)
        ).length;
        ...
        for (const r of zone.reports) {
            const cat = (r as any).confirmed_class || (r as any).predicted_class || 'mixed';
            ...
            if ((r as any).status === 'completed' && (r as any).completed_at) {
                ...
                if (!lastCleanupAt || completedAt > lastCleanupAt) lastCleanupAt = completedAt;
            }
        }
        ...
        const { score, level, norms } = computeScore(
            config, reportCount, unresolvedCount, severitySum, hoursSince
        );
...
export async function getRiskZones(options?: {
    ...
    if (options?.minLevel) {
        const levelMap: Record<RiskLevel, number> = { low: 0, medium: 30, high: 60, critical: 80 };
 ...
 */
export async function getZoneRiskDetails(zoneId: string): Promise<RiskZoneDetail | null> {
    const supabase = createAdminClient();
    const config = await loadConfig();
...
export async function getMismatchZones(): Promise<MismatchZone[]> {
    ...
    const [{ data: riskZones }, { data: benchmarks }] = await Promise.all([
        ...
        supabase.from('government_benchmarks').select('*').order('cleanliness_rank', { ascending: true }),
    ]);
    ...
    const cityBenchmark = benchmarks.find((b: any) =>
    ...
    for (const zone of riskZones) {
        ...
        if (isHighRisk && govIsClean) {
            mismatches.push({
                ...
                mismatch_severity: round(clamp((z.risk_score - 60) / 40 + (govScore - 70) / 30, 0, 1), 3),
            });
            ...
            mismatches.push({
                ...
                mismatch_severity: round(clamp((70 - govScore) / 30 + (30 - z.risk_score) / 30, 0, 1), 3),
            });
        }
    }
...
export async function getRiskSummary() {
    ...
    return {
        ...
        avgScore: round(zones.reduce((sum: number, z: any) => sum + Number(z.risk_score), 0) / zones.length),
        totalUnresolved: zones.reduce((sum: number, z: any) => sum + (z.unresolved_count || 0), 0),
    };
}
