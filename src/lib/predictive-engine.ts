...
const DEFAULT_CONFIDENCE_THRESHOLD = Number(process.env.PREDICTION_CONFIDENCE_THRESHOLD ?? '0.6');
const ACTIVE_PREDICTION_HOURS = Number(process.env.PREDICTION_TARGET_HOURS ?? '24');
const FEEDBACK_WINDOW_HOURS = Number(process.env.PREDICTION_FEEDBACK_HOURS ?? '24');
...
type PredictedReportRow = Database['public']['Tables']['predicted_reports']['Row'] & { metadata?: any; geohash?: string; feedback_count?: number; last_feedback_at?: string; };
type LocationWeightRow = Database['public']['Tables']['location_weights']['Row'] & { geohash?: string; };
...
type PredictionContextReport = Pick<
    WasteReportRow,
    'id' | 'latitude' | 'longitude' | 'created_at' | 'predicted_class' | 'confirmed_class' | 'status'
> & {
...
function normalizeCategory(category: string | null | undefined): WasteCategory {
    const allowed: WasteCategory[] = ['cardboard', 'metal', 'paper', 'plastic', 'glass', 'organic', 'mixed'];
...
function getTemporalStats(reports: PredictionContextReport[]) {
    ...
    for (let index = 1; index < sortedAscending.length; index += 1) {
        gaps.push(hoursBetween(sortedAscending[index].created_at, sortedAscending[index - 1].created_at));
    }
    ...
    return {
        ...
        dayConcentration: reports.length > 0 ? peakWeekday.count / reports.length : 0,
        averageGapHours: average(gaps),
    };
}

function getNextTargetDate(peakHourUtc: number, peakWeekdayUtc: number, dayConcentration: number) {
    const now = new Date();
    ...
    const maxTarget = new Date(now.getTime() + ACTIVE_PREDICTION_HOURS * 60 * 60 * 1000);
...
function buildAdaptiveFeatureScores(cluster: ClusterSummary): AdaptiveFeatureScores {
    const frequencyScore = clamp(
        cluster.reportCount / 8 * 0.25 +
            cluster.momentumScore * 0.45 +
            cluster.recurrenceScore * 0.30,
        0,
        1
    );
    const locationScore = clamp(
        (Number(cluster.locationWeight || 1) / 1.6) * 0.45 +
            ...
            cluster.nearbySupportScore * 0.20,
        0,
        1
    );
    const temporalScore = clamp(
        (cluster.hourConcentration * 0.35) +
            ...
            (cluster.cadenceScore * 0.20) +
            (cluster.trendScore * 0.20),
        0,
        1
    );
...
async function countActualHotspotsForZone(
    ...
    return ((reports ?? []) as Array<{ latitude: number; longitude: number }>)
        .filter((report) => ngeo.encode(report.latitude, report.longitude, 6) === zoneId)
        .length;
}
...
async function recalculateZoneMetrics(supabase: any, zoneId: string) {
    ...
    const nextThreshold = clamp(
        threshold.baseline_threshold - threshold.learning_rate * (pai - threshold.target_pai),
...
async function recordFeedbackCycle(supabase: any, zoneId: string, cycleEndedAt: string) {
    const threshold = await getZoneThreshold(supabase, zoneId);
    const cycleStart = new Date(new Date(cycleEndedAt).getTime() - FEEDBACK_WINDOW_HOURS * 60 * 60 * 1000).toISOString();
    ...
    const verifiedCount = cycleFeedback.filter((row) => row.status === 'verified').length;
    const evaluatedCount = cycleFeedback.length;
    const precision = evaluatedCount > 0 ? verifiedCount / evaluatedCount : 0;
    const actualHotspots = await countActualHotspotsForZone(supabase, zoneId, cycleStart, cycleEndedAt);
    const recall = actualHotspots > 0 ? verifiedCount / actualHotspots : precision;
    ...
    const { data: inserted } = await supabase
        ...
        .insert({
            ...
            recall: round(actualHotspots > 0 ? verifiedCount / actualHotspots : recall, 4),
            f1_score: round(f1, 4),
...
function buildClusters(
    ...
    for (const report of reports) {
        ...
        const category = normalizeCategory(report.confirmed_class || report.predicted_class);
            ...
            clusters.set(geohash, {
                ...
                historicalVerificationRate:
                    (weight?.total_predictions ?? 0) > 0
                        ? (weight?.verified_predictions ?? 0) / Math.max(1, weight?.total_predictions ?? 1)
                        : 0.5,
                reports: [report],
    ...
    const clusterList = Array.from(clusters.values()).map((cluster) => {
        ...
        const ageHours = (now - new Date(cluster.latestReportAt).getTime()) / (1000 * 60 * 60);
        const recurrenceScore = clamp(
            cluster.recent30dCount >= 2 ? cluster.recent7dCount / Math.max(2, cluster.recent30dCount) : 0,
            0,
            1
        );
        ...
        const cadenceScore =
            temporal.averageGapHours === null
                ? 0.35
                : clamp(1 - Math.min(temporal.averageGapHours, 24 * 14) / (24 * 14), 0.2, 1);

        return {
            ...cluster,
            dominantCategory: dominant.category,
            categoryDominance: round(dominant.dominance, 3),
            averageGapHours: temporal.averageGapHours ? round(temporal.averageGapHours, 2) : null,
    ...
    for (const cluster of clusterList) {
        ...
        for (const other of clusterList) {
            ...
            if (distance <= HOTSPOT_NEARBY_RADIUS_KM) {
                nearbySupport += clamp(other.reportCount / 6, 0, 1) * (1 - distance / HOTSPOT_NEARBY_RADIUS_KM);
            }
        }
...
function buildDeterministicPredictions(
    ...
    const calibrationMultiplier = Number(calibration?.current_accuracy_multiplier ?? 0.8);

    return clusters
        .filter((cluster) => {
            const ageHours = hoursBetween(cluster.latestReportAt, new Date().toISOString());
            return (
        ...
        .map<PredictionCandidate>((cluster) => {
            const verificationFactor = clamp(cluster.historicalVerificationRate || 0.5, 0.25, 1);
            ...
            const finalWeight = clamp(
                baseConfidence *
                    ...
                    (1 + urgencyBoost + cluster.trendScore * 0.1 + verificationFactor * 0.05),
                0.18,
                1.7
            );
            ...
            if (cluster.hourConcentration >= 0.3) {
                reasonParts.push(`repeats near ${String(cluster.peakHourUtc).padStart(2, '0')}:00 UTC`);
            }
...
async function getPredictionContext() {
    ...
    const [{ data: reports }, { data: weights }, { data: calibrationRows }] = await Promise.all([
        supabase
            .from('waste_reports')
            .select('id, latitude, longitude, created_at, predicted_class, confirmed_class, neighborhood_id, status')
...
export async function expireMissedPredictions() {
    ...
    for (const prediction of expired) {
        const zoneId = prediction.geohash || ngeo.encode(prediction.latitude, prediction.longitude, 6);
        await supabase
            .from(PREDICTION_TABLE)
            .update({
                status: 'missed',
...
export async function generatePredictions(options?: {
    ...
    const { supabase, reports, weights, calibration } = await getPredictionContext();
    ...
    const scopedReports = options?.neighborhoodId
        ? reports.filter((report) => report.neighborhood_id === options.neighborhoodId)
        : reports;
    ...
    const candidates = buildDeterministicPredictions(clusters, calibration, adaptiveWeights.weights);
    const filteredWithThresholds = await Promise.all(
        candidates.map(async (candidate) => {
            ...
            const conflictsWithLiveReport = activeFieldReports.some((report) => {
                ...
                const reportCategory = normalizeCategory(report.confirmed_class || report.predicted_class);
    ...
    if (selectedRows.length === 0 && filteredWithThresholds.length > 0) {
        selectedRows = [...filteredWithThresholds]
            .filter((row) => Math.max(row.candidate.base_confidence, row.candidate.final_weight) >= 0.34)
            .sort((a, b) => {
                const scoreA = Math.max(a.candidate.base_confidence, a.candidate.final_weight);
                const scoreB = Math.max(b.candidate.base_confidence, b.candidate.final_weight);
                return scoreB - scoreA;
            })
    ...
    const rows = filtered.map((prediction) => ({
        ...
        metadata: {
            ...
            confidence_threshold:
                selectedRows.find((row) => row.candidate.geohash === prediction.geohash)?.threshold ??
                confidenceThreshold,
    ...
    for (const row of rows) {
        const existingMatch = existingActive.find((prediction) => {
            ...
            const sameCategory =
                normalizeCategory(prediction.predicted_category) === row.predicted_category ||
    ...
    if (insertedCount > 0) {
        if (calibration?.id) {
            const nextGenerated = (calibration.total_predictions_generated ?? 0) + insertedCount;
            await supabase
...
export async function updateFeedbackStats(geohash: string, verified: boolean) {
    ...
    const verifiedPredictions = (existing?.verified_predictions ?? 0) + (verified ? 1 : 0);
    ...
    const { data: calibration } = await supabase.from('model_calibration').select('*').limit(1).single();
    if (calibration) {
        const nextVerified = (calibration.total_predictions_verified ?? 0) + (verified ? 1 : 0);
...
export async function verifyPredictionForReport(report: Pick<WasteReportRow, 'id' | 'latitude' | 'longitude' | 'created_at' | 'confirmed_class' | 'predicted_class'>) {
    const supabase = createServiceClient();
    const reportCategory = normalizeCategory(report.confirmed_class || report.predicted_class);
    ...
    const matching = ((activePredictions ?? []) as PredictedReportRow[])
        .map((prediction: PredictedReportRow) => {
            const distance = haversineKm(report.latitude, report.longitude, prediction.latitude, prediction.longitude);
            const predictedCategory = normalizeCategory(prediction.predicted_category);
            const categoryScore =
                predictedCategory === reportCategory ? 1 :
                predictedCategory === 'mixed' ? 0.7 : 0;
            const distanceScore = clamp(1 - distance / LOCATION_RADIUS_KM, 0, 1);
            const timeScore = clamp(
                1 - hoursBetween(report.created_at, prediction.target_date) / FEEDBACK_WINDOW_HOURS,
                0,
                1
            );
        ...
        .filter((entry) => entry.distance <= LOCATION_RADIUS_KM && entry.categoryScore > 0)
    ...
    const zoneId = matching.prediction.geohash || ngeo.encode(matching.prediction.latitude, matching.prediction.longitude, 6);
    const distanceMeters = matching.distance * 1000;
    const timeDeltaMinutes = hoursBetween(report.created_at, matching.prediction.target_date) * 60;
    ...
    await supabase.from('prediction_feedback_records').insert({
        ...
        metadata: {
            matched_category: reportCategory,
            match_score: round(matching.matchScore, 3),
            features: (matching.prediction.metadata as Record<string, unknown>)?.features,
        },
    });
...
export async function getPredictionSummary() {
    ...
    const [{ data: predictions }, { data: calibration }, { data: weights }, { data: thresholds }, { data: cycles }] = await Promise.all([
        supabase.from(PREDICTION_TABLE).select('*').order('final_weight', { ascending: false }),
        supabase.from('model_calibration').select('*').limit(1).single(),
        supabase.from('location_weights').select('*'),
        supabase.from('prediction_thresholds').select('*'),
        supabase.from('prediction_cycle_metrics').select('*').order('cycle_ended_at', { ascending: false }).limit(20),
    ]);

    const typedWeights = (weights ?? []) as LocationWeightRow[];
    const totalLocationPredictions = typedWeights.reduce((sum, row) => sum + (row.total_predictions ?? 0), 0);
    const totalLocationVerified = typedWeights.reduce((sum, row) => sum + (row.verified_predictions ?? 0), 0);
    const accuracy =
    ...
    return {
        ...
        metrics: {
            active: ((predictions ?? []) as PredictedReportRow[]).filter((prediction) => prediction.status === 'active').length,
            verified: ((predictions ?? []) as PredictedReportRow[]).filter((prediction) => prediction.status === 'verified').length,
            missed: ((predictions ?? []) as PredictedReportRow[]).filter((prediction) => prediction.status === 'missed').length,
            accuracy,
