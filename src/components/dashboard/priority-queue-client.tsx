...
import { type PriorityTier, getTierLabel, getSLAColor } from '@/lib/priority-engine';
...
export default function PriorityQueueClient() {
    ...
    const [summary, setSummary] = useState<PrioritySummary>({ total: 0, critical: 0, high: 0, medium: 0, low: 0 });
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                ...
                <Button variant="outline" size="sm" onClick={fetchPriorities} disabled={loading} className="gap-2">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                {[
                    { label: 'Total', count: summary.total, color: 'from-slate-500 to-slate-600', icon: Activity },
                    { label: 'Critical', count: summary.critical, color: 'from-red-500 to-red-600', icon: AlertTriangle },
                    { label: 'High', count: summary.high, color: 'from-amber-500 to-amber-600', icon: Zap },
                    { label: 'Medium', count: summary.medium, color: 'from-blue-500 to-blue-600', icon: Clock },
                    { label: 'Low', count: summary.low, color: 'from-emerald-500 to-emerald-600', icon: Timer },
                    ...
                    >
                        ...
                        {t.value !== 'all' && (
                            <span className="ml-1 opacity-70">
                                {t.value === 'critical' ? summary.critical :
                                 t.value === 'high' ? summary.high :
                                 t.value === 'medium' ? summary.medium : summary.low}
                            </span>
                        )}
                    </button>
                ))}
            ...
            {loading ? (
                ...
                <Card className="border-dashed">
                    <CardContent className="py-12 text-center">
                        ...
                        <p className="font-medium text-muted-foreground">No pending reports</p>
                        <p className="text-xs text-muted-foreground/70 mt-1">
                            {filter !== 'all' ? `No ${filter} priority reports. Try a different filter.` : 'All caught up!'}
                        </p>
                    </CardContent>
                </Card>
            ) : (
                <div className="space-y-3">
                    {reports.map((report) => {
                        const wasteType = report.confirmed_class || report.predicted_class || 'mixed';
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className={`absolute left-0 top-0 bottom-0 w-1 ${
                                    p.tier === 'critical' ? 'bg-red-500' :
                                    p.tier === 'high' ? 'bg-amber-500' :
                                    p.tier === 'medium' ? 'bg-blue-500' : 'bg-emerald-500'
                                }`} />
                                                ...
                                                <div className="flex items-center gap-2 flex-wrap">
                                                    ...
                                                    {p.slaStatus !== 'safe' && (
                                                        ...
                                                        >
                                                            ...
                                                            {p.slaStatus === 'breached' ? 'SLA Breached' :
                                                             p.slaStatus === 'critical' ? `${p.hoursRemaining}h left` :
                                                    ...
                                                    <div className="flex-1 h-1.5 bg-muted rounded-full overflow-hidden max-w-[200px]">
                                                        <div
                                                            className={`h-full rounded-full transition-all duration-500 ${
                                                                p.tier === 'critical' ? 'bg-gradient-to-r from-red-500 to-red-600' :
                                                                p.tier === 'high' ? 'bg-gradient-to-r from-amber-500 to-amber-600' :
                                                                p.tier === 'medium' ? 'bg-gradient-to-r from-blue-500 to-blue-600' :
                                                ...
                                                {isExpanded && (
                                                    <div className="mt-3 pt-3 border-t border-border/50 space-y-2 animate-slide-up">
                                                        <div className="grid grid-cols-5 gap-2">
                                                            {[
                                                                { label: 'Waste', val: p.breakdown.wasteTypeScore },
                                                                { label: 'Age', val: p.breakdown.ageScore },
                                                                { label: 'Qty', val: p.breakdown.quantityScore },
                                                                { label: 'Density', val: p.breakdown.densityScore },
                                                                { label: 'AI Conf', val: p.breakdown.confidenceScore },
                                                            ].map((f) => (
