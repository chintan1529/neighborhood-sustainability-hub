    ...
    <div className="rounded-md border">
      <Table>
        ...
        <TableBody>
          {reports.map((report) => {
            const status =
              REPORT_STATUSES[report.status as keyof typeof REPORT_STATUSES];
                ...
                <TableCell>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="capitalize">
                      {report.confirmed_class || "Processing"}
                    </span>
                  </div>
                </TableCell>
                <TableCell>
                    ...
                    <Dialog>
                      <DialogTrigger asChild>
                        <Button
                          variant="outline"
                          size="sm"
                          className="h-8 gap-1.5 text-xs text-emerald-700 hover:text-emerald-800 hover:bg-emerald-50 border-emerald-200"
                        >
                        ...
                        <div className="w-full h-full flex items-center justify-center p-4 pt-16 pb-12">
                          <BeforeAfterSlider
                            beforeImage={
                              supabase.storage
                                .from("report-photos")
                                .getPublicUrl(report.photo_url).data.publicUrl
                            }
                            afterImage={
                              supabase.storage
                                .from("proofs")
                                .getPublicUrl(report.completion_photo_url).data
                                .publicUrl
                            }
