...
export function NeighborhoodLeaderboard({ data, userNeighborhoodId }: LeaderboardProps) {
                            ...
                            >
                                ...
                                <div className="col-span-4 pl-2 flex flex-col justify-center">
                                    <div className="flex items-center gap-2">
                                        <p className={`font-semibold ${isTop3 ? 'text-foreground' : 'text-muted-foreground'} ${isUserNh ? 'text-primary' : ''}`}>
                                            {neighborhood.name}
                                        </p>
                                        {isUserNh && (
                                            <Badge variant="default" className="bg-primary/20 text-primary border-primary/30 text-[9px] px-1.5 py-0 h-4 min-w-[40px] flex items-center justify-center">
                                                YOURS
                                            </Badge>
                                        )}
                                    </div>
                                ...
                                <div className="col-span-3 text-right font-black tracking-tight flex items-center justify-end gap-1.5">
                                    <TrendingUp className={`w-4 h-4 ${isTop3 ? 'text-emerald-500' : 'text-muted-foreground/50'}`} />
                                    <span className={isTop3 ? 'text-lg text-emerald-600 dark:text-emerald-400' : 'text-foreground'}>
