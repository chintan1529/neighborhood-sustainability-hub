...
export function ChatInterface({ conversationId, basePath = '/resident/messages' }: ChatInterfaceProps) {
    ...
    const fetchMessages = useCallback(async () => {
        try {
            ...
            const senderIds = Array.from(new Set((messagesData || []).map((m: any) => m.sender_id))) as string[];
            ...
            const formattedMessages: MessageWithSender[] = (messagesData || []).map((m: any) => ({
                ...m,
    ...
    const displayName = conversationInfo?.type === 'direct'
            ...
            <div className="border-b p-4 flex items-center gap-3 bg-background">
                ...
                {conversationInfo?.participants[0] && (
                    <Avatar className="h-10 w-10">
                        <AvatarImage src={conversationInfo.participants[0].avatar_url || undefined} />
                        <AvatarFallback>{getInitials(conversationInfo.participants[0].full_name)}</AvatarFallback>
                    </Avatar>
                )}
                <div>
                                            ...
                                            >
                                                <div className={`flex gap-2 max-w-[80%] ${isOwn ? 'flex-row-reverse' : ''}`}>
                                                    ...
                                                    <div>
                                                        ...
                                                        <span className={`text-xs text-muted-foreground mt-1 ${isOwn ? 'text-right block' : ''}`}>
