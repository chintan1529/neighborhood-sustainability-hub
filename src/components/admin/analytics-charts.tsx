...
export function AnalyticsDashboard({
    ...
    const weeklyTrend = kpis.reportsLastWeek > 0
        ? Math.round(((kpis.reportsThisWeek - kpis.reportsLastWeek) / kpis.reportsLastWeek) * 100)
        : 0;
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <KpiCard
                    ...
                    gradient={kpis.slaComplianceRate >= 80 ? 'from-violet-500 to-purple-600' : 'from-red-500 to-rose-600'}
                />
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <KpiCard
                    title="Avg Rating"
                    value={kpis.avgCollectorRating > 0 ? kpis.avgCollectorRating.toFixed(1) : '—'}
                ...
                <KpiCard
                    ...
                    icon={AlertTriangle}
                    gradient={kpis.pendingReports > 10 ? 'from-red-500 to-rose-600' : 'from-slate-500 to-slate-600'}
                />
            </div>
            ...
            <div className="grid gap-6 lg:grid-cols-5">
                ...
                <Card className="lg:col-span-2 border border-border/50">
                    <CardHeader className="pb-2">
                        <CardTitle className="text-sm font-semibold">Waste Categories</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <ResponsiveContainer width="100%" height={260}>
                            <PieChart>
            ...
            {paiHistory.length > 0 && (
                <Card className="border border-border/50">
                    <CardHeader className="pb-2">
                        <CardTitle className="text-sm font-semibold">PAI And Threshold Trend</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <ResponsiveContainer width="100%" height={240}>
                            <AreaChart data={paiHistory}>
                                ...
                                <Tooltip content={<CustomTooltip />} />
                                <Legend iconSize={8} wrapperStyle={{ fontSize: '11px' }} />
                                <Area type="monotone" dataKey="pai" name="PAI" stroke="#8B5CF6" fillOpacity={0.2} fill="#8B5CF6" />
                                <Area type="monotone" dataKey="threshold" name="Threshold" stroke="#06B6D4" fillOpacity={0.12} fill="#06B6D4" />
                            </AreaChart>
                        </ResponsiveContainer>
                    </CardContent>
                </Card>
            ...
            <Card className="border border-border/50">
                ...
                <CardContent>
                    {collectorLeaderboard.length === 0 ? (
                        <p className="text-sm text-muted-foreground text-center py-6">No collector data available yet.</p>
                    ) : (
                        <div className="space-y-3">
                            {collectorLeaderboard.map((collector, i) => (
                                <div key={i} className="flex items-center justify-between gap-4 p-3 rounded-lg bg-muted/30 hover:bg-muted/50 transition-colors">
                                    ...
                                    <div className="flex items-center gap-4">
                                        <div className="text-right">
                                            ...
                                            <p className="text-[10px] text-muted-foreground mt-0.5">
                                                {collector.avgRating > 0 ? collector.avgRating.toFixed(1) : 'No'} rating
                                            </p>
                                        </div>
