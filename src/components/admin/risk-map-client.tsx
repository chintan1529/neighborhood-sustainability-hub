...
export function RiskMapClient({ initialZones, center }: RiskMapProps) {
    const { zones, isConnected, lastUpdated } = useRealtimeRiskZones(initialZones);
    ...
    return (
        <div className="relative">
            ...
            <div className="absolute top-3 right-3 z-[500] flex items-center gap-2">
                {isConnected ? (
                    <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200 gap-1 text-[10px] shadow-sm">
                        <Wifi className="h-3 w-3" /> Live
                    </Badge>
                ) : (
                    <Badge variant="outline" className="bg-red-50 text-red-700 border-red-200 gap-1 text-[10px] shadow-sm">
            ...
            <div className="absolute bottom-4 left-4 z-[500] bg-white/90 dark:bg-gray-900/90 backdrop-blur-sm rounded-lg p-3 shadow-lg border text-xs space-y-1.5">
                <p className="font-semibold text-[10px] uppercase tracking-wider text-muted-foreground mb-2">Risk Level</p>
