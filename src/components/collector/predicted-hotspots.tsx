...
import { BrainCircuit, CheckCircle2, Clock, MapPin, Target, TrendingUp, XCircle } from 'lucide-react';
...
export function PredictedHotspots() {
    ...
    useEffect(() => {
        ...
        async function fetchPredictions() {
            try {
                const response = await fetch('/api/predicted-hotspots?status=active&limit=3');
        ...
        <div className="space-y-4">
            ...
            <div className="grid gap-4 md:grid-cols-3">
                {predictions.map((prediction) => {
                    ...
                    const category = WASTE_CATEGORIES[wasteType as keyof typeof WASTE_CATEGORIES];
                    return (
                        <Card
                            key={prediction.id}
                        ...
                        >
                            <CardHeader className="p-4 pb-2">
                                <div className="flex justify-between items-start gap-2">
                                    <Badge variant="secondary" className="bg-violet-100 text-violet-700 dark:bg-violet-900/40 dark:text-violet-300 border-none">
                                ...
                                <CardDescription className="text-xs line-clamp-2 italic">
                                    {prediction.metadata?.reason || 'Deterministic hotspot scoring found repeat waste activity in this area.'}
                                </CardDescription>
                            </CardHeader>
