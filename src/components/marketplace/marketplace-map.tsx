...
function LocateMeControl() {
  const map = useMap();
  return (
    <div className="leaflet-top leaflet-right" style={{ pointerEvents: 'auto', zIndex: 1000, position: 'absolute', top: '10px', right: '10px' }}>
      <button 
        onClick={(e) => {
            e.preventDefault();
            e.stopPropagation();
...
export default function MarketplaceMap({
  ...
  return (
    <div className={`rounded-xl overflow-hidden border border-border shadow-sm ${className}`} style={{ height }}>
      <MapContainer 
            ...
            >
              <Popup>
                <div className="p-1">
                  ...
                  <p className="text-xs text-muted-foreground">{listing.weight_kg}kg • {listing.category}</p>
                </div>
              </Popup>
            </Marker>
          );
