    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                <Card>
                    <CardHeader className="pb-2"><CardTitle className="text-sm font-medium">My Active Jobs</CardTitle></CardHeader>
                    <CardContent><div className="text-2xl font-bold">{myJobs?.length || 0}</div></CardContent>
                </Card>
                <Card>
                    <CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Pending Requests</CardTitle></CardHeader>
                    <CardContent><div className="text-2xl font-bold">{pendingReports?.length || 0}</div></CardContent>
                </Card>
            </div>
            ...
            <Card className="relative overflow-hidden border border-emerald-200 dark:border-emerald-800 bg-gradient-to-r from-emerald-50 via-teal-50 to-cyan-50 dark:from-emerald-950/30 dark:via-teal-950/20 dark:to-cyan-950/10">
                ...
                <CardContent className="relative py-5 px-6">
                    <div className="flex items-center justify-between gap-4">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <div className="flex items-center gap-2">
                                    <h3 className="font-semibold text-sm">AI Smart Route Optimizer</h3>
                        ...
                        <Button asChild size="sm" className="bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white border-0 shadow-md gap-1.5">
            ...
            {/* Priority Risk Zones */}
            <RiskZonesWidget zones={riskZones || []} role="collector" viewAllHref="/collector/map" />
            ...
            <div className="grid gap-6 md:grid-cols-2">
                ...
                <Card>
                    ...
                    <CardContent>
                        {myJobs?.length === 0 ? (
                            <p className="text-muted-foreground">No active jobs. Claim some from the queue!</p>
                        ) : (
                            <div className="space-y-4">
                                {myJobs?.map(job => (
                                    <div key={job.id} className="border p-3 rounded-lg flex justify-between items-center bg-muted/20">
                                        <div>
                                            <div className="font-medium flex items-center gap-2">
                                                <span className="capitalize">{job.confirmed_class || 'Waste'}</span>
                                                <Badge variant="outline" className="text-blue-600 border-blue-200">
                                                    {job.status === 'in_progress' ? 'Picked Up' : 'Assigned'}
                                                </Badge>
                                            </div>
                                            <p className="text-sm text-muted-foreground truncate max-w-[200px]">{job.address_text}</p>
                                        </div>
                    ...
                    <CardContent>
                        <div className="space-y-4">
                            {pendingReports?.length === 0 ? (
                                <p className="text-muted-foreground">No pending requests in your area.</p>
                            ) : (
                                pendingReports?.map(report => (
                                    <div key={report.id} className="flex justify-between items-center border-b pb-4 last:border-0 last:pb-0">
                                        <div className="space-y-1">
                                            ...
                                            <p className="text-sm text-muted-foreground truncate max-w-[200px]">{report.address_text}</p>
                                        </div>
                                        <form action={async () => {
                                            'use server';
