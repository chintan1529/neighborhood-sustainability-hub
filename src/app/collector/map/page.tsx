...
export default async function CollectorMapPage() {
    ...
    const { data: reports } = await supabase
        .from('waste_reports')
        .select('id, latitude, longitude, status, predicted_class, confirmed_class, address_text')
    ...
    const reportMarkers = ((reports ?? []) as Array<{
        id: string;
        latitude: number;
        longitude: number;
        status: string;
    ...
    const hotspotMarkers = ((predictedHotspots ?? []) as Array<{
        id: string;
        latitude: number;
        longitude: number;
        predicted_category: string | null;
        ...
        <div className="space-y-6 h-full flex flex-col">
            <div className="flex items-center justify-between">
                ...
                <p className="text-muted-foreground md:block hidden">
                    Visualizing {reportMarkers.length} active reports and {hotspotMarkers.length} predicted hotspots
                </p>
            </div>
