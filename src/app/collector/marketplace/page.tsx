...
export default async function CollectorMarketplacePage() {
    ...
    if (profile.verification_status === 'pending') {
        return (
            <div className="container max-w-2xl py-24 text-center animate-in fade-in duration-500">
                ...
                <h2 className="text-2xl font-bold tracking-tight mb-2">Application Under Review</h2>
                <p className="text-muted-foreground">Your request to join the marketplace is being reviewed by the administration. We will notify you once you're approved.</p>
            </div>
        );
    }
            ...
            <div className="container max-w-4xl py-12">
                <div className="bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-900 rounded-lg p-6 flex items-start gap-4 text-red-800 dark:text-red-200">
                    ...
                    <div>
                        ...
                        <p>Your recycler profile is currently {profile.verification_status}. Contact support for more information.</p>
                    </div>
                </div>
            </div>
        );
    ...
    // Sort listings: AI Urgency (simulated via age) first, then distance.
    const sortedListings = (feedResponse.listings || []).sort((a: any, b: any) => {
        return new Date(a.created_at).getTime() - new Date(b.created_at).getTime();
    });

    return (
        <div className="w-full h-[calc(100vh-4rem)] flex flex-col md:flex-row bg-muted/10 overflow-hidden">
