...
import { ArrowRight, Loader2, Store, MapPin, ShieldCheck, Info } from 'lucide-react';
import { WasteCategory } from '@/types/database';
import { requestRecyclerVerification, VerificationFormData } from '@/app/actions/marketplace';
...
export default function OnboardingWizard() {
    ...
    const toggleCategory = (cat: WasteCategory) => {
        setFormData(prev => {
            ...
            if (current.includes(cat)) {
                return { ...prev, accepted_categories: current.filter(c => c !== cat) };
            } else {
    ...
    const handleNext = () => {
        if (step === 1) {
            if (!formData.business_name || (formData.accepted_categories?.length || 0) === 0) {
                toast({ title: "Incomplete", description: "Business name and at least 1 category are required.", variant: "destructive" });
                return;
            }
        }
        setStep(s => s + 1);
    ...
    const handleSubmit = async () => {
        if (!formData.latitude || !formData.longitude) {
            toast({ title: "Location Missing", description: "Please drop a pin on the map.", variant: "destructive" });
            return;
        }

        setIsSubmitting(true);
        try {
            const result = await requestRecyclerVerification(formData as VerificationFormData);
            
            if (result.success) {
                toast({
                    title: "Application Submitted!",
                    description: "Your business is now pending verification. An admin will review it shortly.",
                });
    ...
    return (
        <div className="container max-w-3xl py-12 animate-in fade-in zoom-in-95 duration-500">
            <div className="mb-8 text-center">
                ...
                <h1 className="text-3xl font-bold tracking-tight">Become a Verified Recycler</h1>
                <p className="text-muted-foreground mt-2 max-w-xl mx-auto">
                    Join the local marketplace to buy high-quality, pre-sorted recyclable waste directly from residents.
                </p>
                
                {/* Progress Bar */}
                <div className="flex gap-2 mt-8 max-w-sm mx-auto">
                    {[1, 2].map((s) => (
                        <div key={s} className={`h-2 rounded-full flex-1 transition-colors duration-300 ${s <= step ? 'bg-blue-600' : 'bg-muted'}`} />
                    ))}
                </div>
            </div>
                ...
                {step === 1 && (
                    <>
                        <CardHeader>
                            ...
                            <CardDescription>Tell us about your recycling operations.</CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-6">
                            ...
                            <div className="space-y-3 pt-4 border-t">
                                ...
                                <p className="text-sm text-muted-foreground mb-4">You will only be alerted about listings matching these categories.</p>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    {CATEGORIES.map(cat => (
                                        ...
                                        >
                                            <input
                                                type="checkbox"
                                                checked={formData.accepted_categories?.includes(cat.id) || false}
                                                readOnly
                                                className="w-5 h-5 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                                            />
                                            <Label className="cursor-pointer flex-grow">{cat.label}</Label>
                                        </div>
                                    ))}
                                </div>
                            </div>
                ...
                {step === 2 && (
                    <>
                        <CardHeader>
                            ...
                            <CardDescription>Where do you send your trucks from?</CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-6">
                            <div className="space-y-2">
                                <div className="flex justify-between items-center">
                                    <Label>Service Radius (km)</Label>
                                    <span className="font-bold text-blue-600">{formData.service_radius_km} km</span>
                                </div>
                                <input 
                                    type="range" 
                                    min="2" max="50" step="1"
                                    ...
                                    onChange={(e) => setFormData(p => ({...p, service_radius_km: parseInt(e.target.value)}))}
                                />
                                <p className="text-xs text-muted-foreground mt-1 text-right">Maximum distance you're willing to travel for pickups.</p>
                            </div>
                            ...
                            <div className="space-y-4 pt-4 border-t">
                                ...
                                <div className="p-3 bg-blue-50 dark:bg-blue-950/30 text-blue-700 dark:text-blue-400 rounded-lg flex items-start gap-2 border border-blue-100 dark:border-blue-900 mb-2">
                                    ...
                                    <p className="text-sm">Click anywhere on the map to drop your scrapyard/office location.</p>
                                </div>
                                <div className="rounded-xl overflow-hidden border-2 border-border focus-within:border-blue-500 transition-colors shadow-sm relative">
                                    ...
                                    {formData.latitude && (
                                        <div className="absolute bottom-4 left-4 z-[400] bg-background/90 backdrop-blur-md px-3 py-1.5 rounded-full shadow-lg border border-border text-xs font-semibold flex items-center gap-1.5 animate-in slide-in-from-bottom-2">
                                            <MapPin className="h-3 w-3 text-blue-600" /> Location Captured
                                        </div>
                                    )}
                                </div>
                            </div>
