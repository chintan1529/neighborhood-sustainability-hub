...
const TYPE_CONFIG: Record<ConversationType, { label: string; icon: typeof MessageSquare; color: string }> = {
    direct: { label: 'Direct Message', icon: MessageSquare, color: 'text-blue-500' },
    report: { label: 'Report Discussion', icon: FileText, color: 'text-green-500' },
            ...
            <Card className="text-center py-16">
                <CardContent>
                    ...
                    <p className="text-muted-foreground mb-4">
                        Click "New Chat" above to start a conversation with an admin or collector.
                    </p>
                </CardContent>
            </Card>
        );
        ...
        <div className="space-y-2">
            {conversations.map((conv) => {
                ...
                return (
                    <Link
                        key={conv.id}
                        href={`/resident/messages/${conv.id}`}
                        className="block"
                    >
                        <Card className={`transition-colors hover:bg-muted/50 ${conv.unread_count > 0 ? 'border-primary/50 bg-primary/5' : ''}`}>
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-2">
                                                <h4 className={`font-medium truncate ${conv.unread_count > 0 ? 'text-foreground' : 'text-muted-foreground'}`}>
                                                    {displayName}
                                                </h4>
                                                {conv.type !== 'direct' && (
                                        ...
                                        <div className="flex items-center justify-between mt-1">
                                            <p className={`text-sm truncate ${conv.unread_count > 0 ? 'text-foreground' : 'text-muted-foreground'}`}>
            ...
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p className="text-muted-foreground">
                        Chat with collectors and admins about your reports.
                    </p>
                </div>
