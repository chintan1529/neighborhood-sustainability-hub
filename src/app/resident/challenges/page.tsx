...
export default async function ChallengesPage() {
  ...
  if (challenges && challenges.length > 0) {
    ...
    console.log(
      "Fetching participations for user:",
      user.id,
      "challenges:",
      challengeIds,
    );
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {challenges?.map((challenge) => {
          ...
          const progress = participation
            ? Math.min(
                (participation.current_count / challenge.target_count) * 100,
                100,
              )
            : 0;
            ...
            <Card key={challenge.id} className="flex flex-col">
              <CardHeader>
                <div className="flex justify-between items-start">
                  ...
                  <Badge
                    variant="outline"
                    className="flex items-center gap-1 border-nhs-amber text-nhs-amber"
                  >
                ...
                {isEnrolled && (
                  <div className="space-y-1 pt-2">
                    ...
                    <p className="text-xs text-muted-foreground text-center pt-1">
                      {participation.current_count} of {challenge.target_count}{" "}
                      items reported
                    </p>
                  </div>
                )}
              ...
              <CardFooter className="pt-0">
                {isEnrolled ? (
                  ...
                  <form
                    action={(fd) => void joinChallenge(fd)}
                    className="w-full"
                  >
                    <input
                      type="hidden"
