...
import { Calendar, MapPin, Users, Clock, Award, Video, Building, Sparkles } from 'lucide-react';
...
const TYPE_CONFIG: Record<WorkshopType, { label: string; color: string; icon: string }> = {
...
const LOCATION_CONFIG: Record<LocationType, { label: string; icon: typeof MapPin }> = {
        ...
        <Card className="hover:shadow-lg transition-shadow group">
            ...
            <CardContent className="space-y-4">
                ...
                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                    ...
                    <span>{formatDate(workshop.starts_at)} at {formatTime(workshop.starts_at)}</span>
                </div>
                    ...
                    <div className="flex items-center gap-4">
                        {workshop.max_participants && (
                            <div className="flex items-center gap-1 text-sm">
                                ...
                                <span>{workshop.registrations_count || 0}/{workshop.max_participants}</span>
                            </div>
                        )}
...
async function WorkshopsContent() {
    ...
    if (error) {
        ...
        return (
            <div className="text-center py-10">
                <p className="text-muted-foreground">Unable to load workshops. Please try again later.</p>
            </div>
        );
    }
    ...
    // Split by time
    const now = new Date();
    const upcomingWorkshops = workshops?.filter(w => new Date(w.starts_at) > now) || [];
    const ongoingWorkshops = workshops?.filter(w => {
        const start = new Date(w.starts_at);
        const end = w.ends_at ? new Date(w.ends_at) : new Date(start.getTime() + 2 * 60 * 60 * 1000);
        return start <= now && now <= end;
    }) || [];
    const pastWorkshops = workshops?.filter(w => {
        const end = w.ends_at ? new Date(w.ends_at) : new Date(new Date(w.starts_at).getTime() + 2 * 60 * 60 * 1000);
        return end < now;
    }) || [];
                ...
                {upcomingWorkshops.length === 0 ? (
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {upcomingWorkshops.map((workshop) => (
                            <WorkshopCard
                                key={workshop.id}
                                workshop={{ ...workshop, registrations_count: countMap[workshop.id] || 0 }}
                ...
                {myWorkshops.length === 0 ? (
                    <Card className="text-center py-10">
                        <CardContent>
                            ...
                            <p className="text-sm text-muted-foreground mt-2">Browse upcoming workshops and join one!</p>
                        </CardContent>
                    </Card>
                ) : (
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                {pastWorkshops.length === 0 ? (
                    <Card className="text-center py-10">
                        <CardContent>
                            ...
                            <p className="text-muted-foreground">No past workshops to show.</p>
                        </CardContent>
                    </Card>
                ) : (
        ...
        <div className="container mx-auto py-8 px-4 max-w-6xl">
            ...
            <div className="mb-8">
                ...
                <p className="text-muted-foreground">
                    Join community events, learn new skills, and earn points while making a positive impact!
                </p>
            </div>
