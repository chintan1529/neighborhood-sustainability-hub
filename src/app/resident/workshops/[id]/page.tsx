...
import { ArrowLeft, Calendar, MapPin, Users, Award, Video, Building, Check, AlertCircle, Leaf } from 'lucide-react';
...
export default async function WorkshopDetailPage({ params }: PageProps) {
    ...
    const locationConfig = LOCATION_CONFIG[workshop.location_type as LocationType];
    const LocationIcon = locationConfig.icon;

    const isFull = Boolean(workshop.max_participants && (registrationsCount || 0) >= workshop.max_participants);
    ...
    return (
        <div className="container mx-auto py-8 px-4 max-w-4xl">
            ...
            <Card className="mb-6">
                <CardHeader>
                    <div className="flex items-start justify-between mb-4">
                        <Badge className={`${typeConfig.color} text-white text-sm px-3 py-1`}>
                            {typeConfig.icon} {typeConfig.label}
                        </Badge>
                        <div className="flex items-center gap-2">
                            <Badge variant="outline" className="text-primary font-bold">
                                <Award className="h-4 w-4 mr-1" />
                                +{workshop.points_reward} points
                            </Badge>
                        </div>
                    </div>
                            ...
                            <div>
                                ...
                                {workshop.ends_at && (
                                    <p className="text-sm text-primary font-medium">
                                        Duration: {getDuration(workshop.starts_at, workshop.ends_at)}
                                    </p>
                                )}
                            </div>
                        </div>
                    ...
                    <div className="flex items-center justify-between p-4 bg-primary/5 rounded-lg">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="font-medium">
                                ...
                                {workshop.max_participants && ` / ${workshop.max_participants} spots`}
                            </span>
                        </div>
            ...
            {workshop.description && (
                <Card className="mb-6">
                    <CardHeader>
                        <CardTitle className="text-xl">About This {typeConfig.label}</CardTitle>
                    </CardHeader>
                    <CardContent>
