...
const getStatusBadge = (status: ListingStatus) => {
    switch(status) {
        ...
        case 'negotiating': return <Badge variant="secondary" className="bg-orange-500 hover:bg-orange-600 text-white">Negotiating</Badge>;
        case 'accepted': return <Badge variant="outline" className="text-green-600 border-green-600">Accepted</Badge>;
        case 'picked_up': return <Badge variant="outline" className="text-purple-600 border-purple-600">Logistics phase</Badge>;
        case 'completed': return <Badge variant="secondary" className="bg-emerald-100 text-emerald-800 hover:bg-emerald-200">Completed</Badge>;
        ...
        <div className="container max-w-5xl py-8 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-muted-foreground mt-1">Manage your recyclable waste listings and offers.</p>
                </div>
                <Button asChild size="lg" className="shadow-lg shadow-green-500/20 hover:shadow-green-500/40 transition-shadow">
            ...
            {!hasListings ? (
                <Card className="flex flex-col items-center justify-center p-12 text-center border-dashed border-2 bg-muted/30">
                    ...
                    <h3 className="text-xl font-semibold mb-2">You haven't listed anything yet!</h3>
                    <p className="text-muted-foreground max-w-sm mb-6">
                        Turn your segregated recyclable waste into real value. List plastic, paper, or e-waste for nearby recyclers to bid on.
                    </p>
                    <Button asChild variant="outline">
                        <Link href="/resident/marketplace/create">Create your first listing</Link>
                    </Button>
                </Card>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {listings.map((listing: any) => {
                        const pendingOffers = listing.offers?.filter((o: any) => o.status === 'pending') || [];
                        const topOffer = pendingOffers.sort((a: any, b: any) => b.price_offered - a.price_offered)[0];

                        return (
                            <Card key={listing.id} className="overflow-hidden hover:shadow-md transition-shadow flex flex-col">
                                ...
                                <CardContent className="pt-4 flex-grow space-y-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-2 text-sm">
                                        <div className="bg-muted/50 p-2 rounded-md">
                                            <p className="text-muted-foreground text-xs">You expect</p>
                                            <p className="font-semibold">{listing.expected_price ? formatMoney(listing.expected_price) : 'Open bid'}/kg</p>
                                        </div>
                                        <div className="bg-blue-50/50 dark:bg-blue-950/20 p-2 rounded-md border border-blue-100 dark:border-blue-900">
                                            ...
                                            <p className="font-semibold text-blue-700 dark:text-blue-300">
                                                {listing.ai_suggested_price ? formatMoney(listing.ai_suggested_price) : 'N/A'}/kg
                                            </p>
                                        </div>
                                    </div>
                                                ...
                                                <>
                                                    <div className="flex justify-between items-center mb-1">
                                                        <span className="text-sm font-medium flex items-center gap-1">
                                                            <Clock className="w-3 h-3 text-orange-500"/> {pendingOffers.length} Active Bids
                                                        </span>
                                                        <span className="text-lg font-bold text-green-600">{formatMoney(topOffer.price_offered)}</span>
                                                    </div>
                                                    <p className="text-xs text-muted-foreground line-clamp-1">
                                                        Top bidder: {topOffer.recycler?.business_name} (⭐{topOffer.recycler?.total_rating})
                                                    </p>
                                                </>
                                            ) : (
                                                <div className="text-center py-2 relative z-10">
                                                    <p className="text-sm text-muted-foreground italic">Waiting for recyclers...</p>
                                                </div>
                                            )}
                                    ...
                                    {listing.status === 'accepted' && (
                                        <div className="flex items-center gap-2 p-3 bg-green-50 dark:bg-green-950/30 text-green-700 dark:text-green-400 rounded-lg border border-green-200 dark:border-green-900">
                                            ...
                                            <div className="text-sm">
                                                ...
                                                <p className="text-xs opacity-90">Awaiting scheduling & pickup.</p>
                                            </div>
                                        </div>
                                    )}
                                ...
                                <CardFooter className="pt-0 pb-4 px-6 border-t mt-auto">
                                    <Button asChild variant={pendingOffers.length > 0 && listing.status === 'active' ? 'default' : 'secondary'} className="w-full mt-4">
                                        <Link href={`/resident/marketplace/${listing.id}`}>
                                            {listing.status === 'active' ? 'Review Offers' : 'View Details'}
                                        </Link>
                                    </Button>
                                </CardFooter>
                            </Card>
                    ...
                    <Card className="flex flex-col items-center justify-center p-6 text-center border-dashed border-2 bg-muted/10 hover:bg-muted/30 transition-colors min-h-[300px]">
                        ...
                        <p className="text-sm text-muted-foreground mb-6">List additional recyclable waste for incoming bids.</p>
                        <Button asChild variant="outline" className="w-full">
                            <Link href="/resident/marketplace/create">Post Another Listing</Link>
                        </Button>
                    </Card>
                </div>
            )}
