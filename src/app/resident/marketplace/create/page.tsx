...
export default function CreateListingWizard() {
    ...
    const handleNext = () => {
        if (step === 1 && (!formData.category || !formData.weight_kg)) {
            toast({ title: "Incomplete", description: "Select category and weight.", variant: "destructive" });
            return;
        }
        if (step === 2 && (!formData.latitude || !formData.longitude)) {
            toast({ title: "Location Missing", description: "Drop a pin on the map to set pickup location.", variant: "destructive" });
            return;
        }
        setStep(s => s + 1);
    };
    ...
    const handleSubmit = async () => {
        if (!formData.title || !formData.address_text) {
            toast({ title: "Final Details", description: "Title and Address are required.", variant: "destructive" });
            return;
        }
        ...
        try {
            ...
            if (result.success) {
                toast({
                    ...
                    description: "Your waste is now live on the marketplace. AI price guidance has been applied.",
                });
    ...
    return (
        <div className="container max-w-3xl py-8 animate-in fade-in zoom-in-95 duration-500">
            <div className="mb-8">
                ...
                <p className="text-muted-foreground mt-2">Follow the steps to list your recyclable waste and get bids from verified collectors.</p>
                
                {/* Progress Bar */}
                <div className="flex gap-2 mt-6">
                    {[1, 2, 3].map((s) => (
                        <div key={s} className={`h-2 rounded-full flex-1 transition-colors duration-300 ${s <= step ? 'bg-green-500' : 'bg-muted'}`} />
                    ))}
                </div>
            </div>
            ...
            <Card className="shadow-lg border-muted/50">
                {step === 1 && (
                    <>
                        <CardHeader>
                            ...
                            <CardDescription>Selecting accurate categories helps recyclers bid fairly.</CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-6">
                            <div className="space-y-2">
                                <Label>Waste Category</Label>
                                <Select onValueChange={(val) => setFormData(p => ({ ...p, category: val as WasteCategory }))} value={formData.category}>
                            ...
                            <div className="space-y-2 relative">
                                ...
                                <div className="relative">
                                    <Input 
                                        ...
                                        onChange={(e) => setFormData(p => ({...p, weight_kg: parseFloat(e.target.value)}))}
                                    />
                                    <div className="absolute right-4 top-1/2 -translate-y-1/2 text-muted-foreground font-medium">kg</div>
                                </div>
                            </div>
                        </CardContent>
                    </>
                ...
                {step === 2 && (
                    <>
                        <CardHeader>
                            ...
                            <CardDescription>Drop a pin exactly where the recycler should back up their truck.</CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            <div className="p-3 bg-blue-50 dark:bg-blue-950/30 text-blue-700 dark:text-blue-400 rounded-lg flex items-start gap-2 border border-blue-100 dark:border-blue-900 mb-2">
                                ...
                                <p className="text-sm">Click anywhere on the map to drop your pickup pin. Zoom in for higher accuracy.</p>
                            </div>
                            ...
                            <div className="rounded-xl overflow-hidden border-2 border-border focus-within:border-green-500 transition-colors shadow-sm relative">
                                ...
                                {formData.latitude && (
                                    <div className="absolute bottom-4 left-4 z-[400] bg-background/90 backdrop-blur-md px-3 py-1.5 rounded-full shadow-lg border border-border text-xs font-semibold flex items-center gap-1.5 animate-in slide-in-from-bottom-2">
                                        <MapPin className="h-3 w-3 text-green-500" /> Pin Dropped Successfully
                                    </div>
                                )}
                            </div>
                        </CardContent>
                ...
                {step === 3 && (
                    <>
                        <CardHeader>
                            ...
                            <CardDescription>Give it a catchy title so recyclers notice it in the feed.</CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-6">
                            <div className="space-y-2">
                            ...
                            <div className="space-y-2">
                                ...
                                <Input 
                                    ...
                                    onChange={(e) => setFormData(p => ({...p, address_text: e.target.value}))}
                                />
                            </div>
                                ...
                                <div className="p-4 bg-muted/40 border border-border rounded-lg mb-2 flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
                                    <div>
                                        <p className="text-sm font-semibold flex items-center gap-2">
                                            <span className="bg-primary/10 text-primary p-1 rounded">🤖 AI Suggestion</span>
                                        </p>
                                        <p className="text-xs text-muted-foreground mt-1">Based on recent {formData.category} deals in your 20km radius.</p>
                                    </div>
                                    <div className="text-right">
                                        <p className="text-2xl font-bold text-green-600">Dynamic</p>
                                        <p className="text-[10px] text-muted-foreground">Calculated on submit</p>
                                    </div>
                                </div>
                                <div className="relative">
                                    ...
                                    <Input 
                                        ...
                                        onChange={(e) => setFormData(p => ({...p, expected_price: parseFloat(e.target.value)}))}
                                    />
                                </div>
                            </div>
                        </CardContent>
                ...
                <CardFooter className="bg-muted/10 border-t p-6 flex justify-between">
                    ...
                    {step < 3 ? (
                        <Button onClick={handleNext} className="bg-green-600 hover:bg-green-700 text-white min-w-[120px]">
