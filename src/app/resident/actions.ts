...
export async function submitCollectorReview(reportId: string, rating: number, review: string) {
    try {
        ...
        const report = reportData as { id: string; status: string; collector_rating: number | null } | null;
        ...
        if (report.status !== 'completed') {
            return { success: false, error: 'You can only review completed reports.' };
        }

        if (report.collector_rating) {
            return { success: false, error: 'You have already rated this collector.' };
        }
