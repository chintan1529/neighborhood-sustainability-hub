...
export default async function AdminDashboard() {
    ...
    const [
        ...
        admin.from('waste_reports').select('*', { count: 'exact', head: true }).eq('status', 'completed'),
        admin.from('waste_reports').select('*', { count: 'exact', head: true }).eq('status', 'pending'),
        admin.from('profiles').select('*', { count: 'exact', head: true }).eq('role', 'resident'),
        admin.from('profiles').select('*', { count: 'exact', head: true }).eq('role', 'collector'),
    ]);
    ...
    if (completedData && completedData.length > 0) {
        ...
        avgCompletionHours = Math.round((hours.reduce((a, b) => a + b, 0) / hours.length) * 10) / 10;
    ...
    const [{ count: reportsThisWeek }, { count: reportsLastWeek }] = await Promise.all([
        admin.from('waste_reports').select('*', { count: 'exact', head: true }).gte('created_at', oneWeekAgo.toISOString()),
        admin.from('waste_reports').select('*', { count: 'exact', head: true }).gte('created_at', twoWeeksAgo.toISOString()).lt('created_at', oneWeekAgo.toISOString()),
    ]);
    ...
    if (ratedReports && ratedReports.length > 0) {
        avgCollectorRating = ratedReports.reduce((a, r: any) => a + r.collector_rating, 0) / ratedReports.length;
    }
    ...
    if (collectors && collectors.length > 0) {
        ...
        const collectorStats: Record<string, { jobs: number; totalH: number; totalRating: number; ratingCount: number }> = {};
        (collectorReports || []).forEach((r: any) => {
            if (!r.assigned_to) return;
            if (!collectorStats[r.assigned_to]) {
                collectorStats[r.assigned_to] = { jobs: 0, totalH: 0, totalRating: 0, ratingCount: 0 };
            }
            const s = collectorStats[r.assigned_to];
            s.jobs++;
            if (r.completed_at && r.created_at) {
                s.totalH += (new Date(r.completed_at).getTime() - new Date(r.created_at).getTime()) / (1000 * 60 * 60);
            }
        ...
        collectorLeaderboard = collectors
            .map((c: any) => {
                const s = collectorStats[c.id] || { jobs: 0, totalH: 0, totalRating: 0, ratingCount: 0 };
                return {
                    name: c.full_name || 'Unknown',
                    completedJobs: s.jobs,
                    avgRating: s.ratingCount > 0 ? Math.round((s.totalRating / s.ratingCount) * 10) / 10 : 0,
    ...
    // ── Resolution rate ──
    const resolutionRate = (totalReports || 0) > 0
    ...
    const paiHistory = ((predictionSummary.metrics as any).paiHistory || []).map((row: any) => ({
    ...
    const routeEfficiencyGain = (routeRuns || []).length > 0
    ...
    const feedbackMetrics = {
        ...
        avgThreshold: typedThresholds.length > 0
            ? typedThresholds.reduce((sum, row) => sum + row.threshold, 0) / typedThresholds.length
            : 0.65,
        verifiedPredictions: Number((predictionSummary.metrics as any).verified ?? 0),
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Priority Risk Zones */}
            <RiskZonesWidget zones={riskZones || []} role="admin" viewAllHref="/admin/intelligence" />
        </div>
    );
}
