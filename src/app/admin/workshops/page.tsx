    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-3xl font-bold tracking-tight">Workshops Management</h2>
                    <p className="text-muted-foreground">Create and manage community workshops, campaigns, and events</p>
                </div>
                <WorkshopFormDialog />
            </div>
                        ...
                        <TableBody>
                            {workshopList.map((workshop: any) => (
                                <TableRow key={workshop.id}>
                                    <TableCell>
                                        <div className="font-medium max-w-[250px]">{workshop.title}</div>
                                        <div className="text-xs text-muted-foreground truncate max-w-[250px]">
                                            {workshop.short_description || workshop.description}
                                        </div>
                                        {workshop.tags && workshop.tags.length > 0 && (
                                            <div className="flex flex-wrap gap-1 mt-1">
                                                {workshop.tags.slice(0, 3).map((tag: string, i: number) => (
                                    ...
                                    <TableCell>
                                        <div className="text-sm">
                                            <span>{locationIcons[workshop.location_type] || ''} {workshop.location_type}</span>
                                        </div>
                                        {workshop.location && (
                                    ...
                                    <TableCell className="text-xs">
                                        ...
                                        {workshop.ends_at && (
                                            <div className="text-muted-foreground">to {formatDate(workshop.ends_at)}</div>
                                        )}
                                    </TableCell>
                                    <TableCell>
                                    ...
                                    <TableCell>
                                        <div className="flex items-center gap-2">
                                            ...
                                            {(workshop.status === 'upcoming' || workshop.status === 'draft') && (
                                                <form action={(fd) => void updateWorkshopStatus(fd)}>
                                                    <input type="hidden" name="workshopId" value={workshop.id} />
