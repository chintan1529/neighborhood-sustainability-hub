...
export default function RecyclerTable({ recyclers }: { recyclers: any[] }) {
  ...
  const handleVerify = async (
    id: string,
    newStatus: Exclude<VerificationStatus, "pending">,
  ) => {
    setProcessingId(id);
                ...
                >
                    ...
                    <div className="text-xs mt-1 space-x-1">
                      {recycler.accepted_categories
                        ...
                        .map((cat: string) => (
                          <span
                            key={cat}
                            className="bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300 px-1.5 py-0.5 rounded text-[10px] uppercase font-bold tracking-wider"
                          >
                            {cat}
                          </span>
                        ))}
                      {recycler.accepted_categories?.length > 3 && (
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="font-medium">
                      {recycler.profiles?.full_name || "No Name"}
                    </div>
                    <div className="text-muted-foreground">
                      {recycler.profiles?.phone || "No Phone"}
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center gap-1">
                      <span className="text-yellow-500 font-bold">
                        ★{recycler.total_rating.toFixed(1)}
                      </span>
                      <span className="text-xs text-muted-foreground">
                        ({recycler.review_count})
                      </span>
                    </div>
                    <p className="text-xs text-muted-foreground mt-1">
                      {recycler.service_radius_km}km Radius
                    </p>
                  </td>

                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    {getStatusBadge(
                      recycler.verification_status as VerificationStatus,
                    )}
                  </td>
                      ...
                      <div className="flex justify-end gap-2">
                        {recycler.verification_status !== "approved" && (
                          <Button
                            ...
                            onClick={() =>
                              handleVerify(recycler.id, "approved")
                            }
                          >
                        ...
                        {recycler.verification_status === "pending" && (
                          <Button
                            ...
                            onClick={() =>
                              handleVerify(recycler.id, "rejected")
                            }
                          >
                        ...
                        {recycler.verification_status === "approved" && (
                          <Button
                            ...
                            onClick={() =>
                              handleVerify(recycler.id, "suspended")
                            }
                          >
