...
import {
    Warehouse, MapPin, Clock, Phone, Truck, TrendingUp,
    CheckCircle2, AlertTriangle, Package, Zap
} from 'lucide-react';
...
const STATUS_CONFIG = {
    low: { color: 'bg-emerald-500', text: 'text-emerald-700', bg: 'bg-emerald-50 dark:bg-emerald-950/30', label: 'Low Load', icon: CheckCircle2 },
    medium: { color: 'bg-amber-500', text: 'text-amber-700', bg: 'bg-amber-50 dark:bg-amber-950/30', label: 'Moderate', icon: TrendingUp },
    high: { color: 'bg-red-500', text: 'text-red-700', bg: 'bg-red-50 dark:bg-red-950/30', label: 'Near Capacity', icon: AlertTriangle },
};
...
const WASTE_COLORS: Record<string, string> = {
    ...
    cardboard: 'bg-amber-100 text-amber-700 dark:bg-amber-950/40 dark:text-amber-400',
...
export function TransferStationsClient({
    ...
    const stationMetrics = useMemo(() => {
        const metrics = new Map<string, { reportCount: number; estimatedTons: number; categories: Record<string, number> }>();
        ...
        recentReports.forEach(report => {
            ...
            stations.forEach(s => {
                const d = haversine(report.latitude, report.longitude, s.latitude, s.longitude);
                if (d < minDist) {
                    minDist = d;
                    nearest = s.id;
                }
        ...
        <div className="space-y-6">
            ...
            <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                    Intermediate depots that minimize long-distance waste hauling across the city
                </p>
            </div>
                ...
                <Card className="border-blue-100 dark:border-blue-900/30">
                    <CardContent className="pt-5 pb-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-2xl font-bold">{totalCapacity}<span className="text-sm font-normal text-muted-foreground"> t</span></p>
                ...
                <Card className="border-emerald-100 dark:border-emerald-900/30">
                    <CardContent className="pt-5 pb-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-2xl font-bold">{avgUtilization}<span className="text-sm font-normal text-muted-foreground">%</span></p>
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                {stations.map(station => {
                    ...
                    const loadPct = station.capacity_tons > 0 ? (station.current_load_tons / station.capacity_tons) * 100 : 0;
                    const status = getLoadStatus(station.current_load_tons, station.capacity_tons);
                    ...
                    return (
                        <Card
                            ...
                            className={`relative overflow-hidden cursor-pointer transition-all duration-300 hover:shadow-lg ${
                                isSelected ? 'ring-2 ring-orange-500 shadow-lg' : 'hover:border-orange-200 dark:hover:border-orange-900/40'
                            ...
                            <CardContent className="space-y-4">
                                ...
                                <div>
                                    <div className="flex justify-between text-xs mb-1.5">
                                        ...
                                        <span className="font-semibold">{station.current_load_tons}t / {station.capacity_tons}t</span>
                                    </div>
                                ...
                                {isSelected && metrics && (
                                    <div className="pt-3 border-t space-y-2 animate-in slide-in-from-top-2 duration-300">
                                        <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">7-Day Activity</p>
                                        <div className="grid grid-cols-2 gap-3">
                                            <div className="bg-muted/50 rounded-lg p-2.5 text-center">
                                                <p className="text-lg font-bold">{metrics.reportCount}</p>
                                                <p className="text-[10px] text-muted-foreground">Reports Served</p>
                                            </div>
                                            <div className="bg-muted/50 rounded-lg p-2.5 text-center">
                                                <p className="text-lg font-bold">{metrics.estimatedTons.toFixed(1)}<span className="text-xs font-normal">t</span></p>
                                                <p className="text-[10px] text-muted-foreground">Est. Throughput</p>
                                            </div>
                                        </div>
                                        {station.notes && (
                                            <p className="text-xs text-muted-foreground italic">📝 {station.notes}</p>
                                        )}
                                    </div>
                                )}
                            </CardContent>
