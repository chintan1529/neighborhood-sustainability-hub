...
export default async function AdminReviewsPage() {
    ...
    const avgRating = reviews && reviews.length > 0 
        ? (reviews.reduce((acc, r) => acc + (r.collector_rating || 0), 0) / reviews.length).toFixed(1)
        : '0.0';
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-3xl font-extrabold tracking-tight">Collector Reviews</h2>
                ...
                <div className="flex items-center gap-3">
                    <div className="flex items-center gap-1 bg-amber-50 text-amber-700 dark:bg-amber-950/30 dark:text-amber-400 px-3 py-1.5 rounded-full text-sm font-medium border border-amber-200 dark:border-amber-900/50">
                        ...
                        <span className="text-amber-700/60 dark:text-amber-400/60 ml-1">({reviews?.length || 0} reviews)</span>
                    </div>
                </div>
            </div>
                                ...
                                reviews.map((review: any) => (
                                    <TableRow key={review.id} className="hover:bg-muted/30">
                                        <TableCell className="text-sm text-muted-foreground">
                                            {review.completed_at ? formatDate(review.completed_at) : 'Unknown'}
                                        </TableCell>
