...
export async function createListing(input: CreateListingInput) {
    ...
    try {
        ...
        const { data: suggestedPrice, error: rpcError } = await (supabase as any).rpc('get_suggested_price', {
...
export async function getMarketplaceFeed(options: { radius_km?: number, category?: WasteCategory }) {
    const supabase = await createClient();
    const { data: { user } } = await supabase.auth.getUser();
    ...
    try {
        ...
        if (!profile || (profile as any).verification_status !== 'approved') {
            return { success: false, error: 'Only verified recyclers can access the feed' };
        }
...
export async function makeOffer(listingId: string, priceOffered: number, proposedPickupTime: string) {
    const supabase = await createClient();
    const { data: { user } } = await supabase.auth.getUser();
...
export async function acceptOffer(offerId: string) {
    ...
    try {
        ...
        const { data: transactionId, error } = await (supabase as any).rpc('accept_marketplace_offer', {
            p_offer_id: offerId,
...
export async function completeTransaction(transactionId: string, paymentMode: 'cash' | 'upi' | 'bank_transfer') {
    const supabase = await createClient();
    const { data: { user } } = await supabase.auth.getUser();
...
export async function verifyRecycler(recyclerId: string, status: 'approved' | 'rejected' | 'suspended') {
    const supabase = await createClient();
    const { data: { user } } = await supabase.auth.getUser();
...
export async function requestRecyclerVerification(data: VerificationFormData) {
    ...
    try {
        ...
        revalidatePath('/collector/marketplace');
        revalidatePath('/admin/recyclers');
        return { success: true };
    } catch (error: any) {
