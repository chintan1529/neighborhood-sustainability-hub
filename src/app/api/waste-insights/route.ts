        ...
        if (!reports || reports.length === 0) {
            return NextResponse.json({
                ...
                insight: {
                    ...
                    message: 'Submit your first waste report to receive personalized waste reduction insights powered by AI.',
                    tips: [
        ...
        if (hfToken) {
            try {
                ...
                const categoryBreakdown = Object.entries(catMap)
                    .map(([cat, count]) => `${cat}: ${count} reports (${Math.round((count / totalReports) * 100)}%)`)
                    .join(', ');
...
No other text.`,
                ...
                if (jsonMatch) {
                    ...
                    return NextResponse.json({
                        ...
                        insight: {
                            ...
                            message: parsed.message || 'Keep reporting to get better insights!',
                            tips: parsed.tips || [],
        ...
        const percentage = Math.round(((topCategory?.[1] || 0) / totalReports) * 100);
