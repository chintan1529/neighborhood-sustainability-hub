...
export async function GET(request: NextRequest) {
    try {
        ...
        const query = supabase
            .from('waste_reports')
            .select('id, status, predicted_class, confirmed_class, prediction_confidence, latitude, longitude, address_text, quantity_estimate, created_at, notes')
            .order('created_at', { ascending: true });

        if (statusFilter === 'all') {
            query.in('status', ['pending', 'assigned', 'in_progress']);
        } else {
            query.eq('status', statusFilter as import('@/types/database').ReportStatus);
        }
        ...
        // Apply tier filter
        if (tierFilter) {
            scoredReports = scoredReports.filter((r) => r.priority.tier === tierFilter);
        }

        // Summary counts
        const summary = {
            total: scoredReports.length,
            critical: scoredReports.filter((r) => r.priority.tier === 'critical').length,
