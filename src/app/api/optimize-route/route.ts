...
function getReportPriorities(reports: ReportLocation[]): Map<string, { score: number; reason: string }> {
...
function buildDistanceMatrix(
    ...
    const allPoints = [start, ...reports.map((r) => ({ latitude: r.latitude, longitude: r.longitude }))];
    ...
    try {
        ...
        if (predictions) {
            predictions.forEach((p) => {
                ...
                const exists = initialReports.some(r => 
                    haversineDistance(r.latitude, r.longitude, p.latitude, p.longitude) < 0.2
                );
        ...
        try {
            ...
            const { data: transferStations } = await adminClient
                ...
                .select('id, name, latitude, longitude, address_text, capacity_tons, current_load_tons')
                .eq('is_active', true);
            ...
            if (transferStations && transferStations.length > 0) {
                ...
                if (routeSpan > 5) {
                    ...
                    const viable = transferStations
                        ...
                        .map(ts => ({
                            ...ts,
                            dist: haversineDistance(centroidLat, centroidLng, ts.latitude, ts.longitude)
                        }))
                        .sort((a, b) => a.dist - b.dist);

                    if (viable.length > 0) {
                        const best = viable[0];
                        const alreadyInjected = reports.some(r => r.id === `ts-${best.id}`);
                        if (!alreadyInjected) {
                            reports.push({
                                id: `ts-${best.id}`,
                            ...
                            console.log(`[Route] Injected transfer station: ${best.name} (${best.dist.toFixed(1)}km from centroid)`);
                        }
                    }
                }
            }
        } catch (tsErr) {
            console.warn('[Route] Transfer station lookup failed, continuing without:', tsErr);
        }
        ...
        const priorityArray = [0, ...reports.map((r) => priorities.get(r.id)?.score || 0.5)];
        ...
        for (let i = 0; i < orderedStops.length; i++) {
            ...
            const priority = priorities.get(report.id) || { score: 0.5, reason: 'standard' };
        ...
        for (let i = 0; i < reports.length; i++) {
            ...
            naiveDistance += haversineDistance(prevLat, prevLng, reports[i].latitude, reports[i].longitude);
        }

        const timeSaved = Math.max(0, Math.round(((naiveDistance - cumulativeDistance) / 15) * 60));
        ...
        try {
            ...
            await admin.from('route_run_metrics').insert({
                ...
                distance_saved_km: Math.round((naiveDistance - cumulativeDistance) * 100) / 100,
                total_time_minutes: Math.round(cumulativeTime),
                time_saved_minutes: timeSaved,
                predicted_stop_count: reports.filter((report) => report.status === 'predicted').length,
                optimization_method: 'nearest-neighbor + 2-opt with AI priority and predictive hotspot injection',
            });
        } catch (metricError) {
        ...
        return NextResponse.json({
            ...
            route: {
                ...
                timeSaved,
                distanceSaved: Math.round((naiveDistance - cumulativeDistance) * 100) / 100,
                transferStationsUsed: reports.filter(r => r.isTransferStation).length,
                optimizationMethod: 'nearest-neighbor + 2-opt with AI priority, predictive hotspot injection, and transfer station waypoints',
            },
        });
    } catch (error: any) {
