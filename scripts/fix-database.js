...
async function fixDatabase() {
    ...
    if (!existingNeighborhoods || existingNeighborhoods.length === 0) {
        ...
        console.log(`✅ Created neighborhood: ${newNeighborhood.name} (${neighborhoodId})`);
    } else {
        neighborhoodId = existingNeighborhoods[0].id;
        console.log(`✅ Found existing neighborhood: ${existingNeighborhoods[0].name} (${neighborhoodId})`);
    }
    ...
    if (profileCheck && profileCheck.length > 0) {
        console.warn(`⚠️  ${profileCheck.length} profiles still have NULL neighborhood.`);
    } else {
